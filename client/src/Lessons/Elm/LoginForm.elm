module Lessons.Elm.LoginForm exposing (lesson)

import Lesson exposing (FileType(..), LessonId(..))
import Phosphor exposing (placeholder)
...
emailInput email =
  input [onInput EmailInput, value email, placeholder "email"] []

passwordInput pw =
  input [onInput PasswordInput, value model.email, placeholder "password"] []
